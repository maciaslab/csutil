...
def searchEncode(searchstring):
    ...
    for item in graph:
        ...
        results.append([item.get("accession", ""), assemblies, item.get(
            "target", "").split("/")[2], item.get("biosample_summary", "")])
    ...
    print tabulate(results, ["Accession", "Assemblies", "Target",
                             "Biosample"], tablefmt="rst").encode('utf-8')
    return
...
def localEncode():
    ...
    print tabulate(results, ["Filename","Accession", "Dataset", "Assembly",
                             "File type","Output Type","Description"], tablefmt="rst").encode('utf-8')
def fileinfoEncodeMini(datafile):
    ...
    response2 = requests.get(URL, headers=HEADERS)
    response_json_dict2 = response2.json()
    return [response_json_dict.get("accession", ""),response_json_dict.get("dataset", "").split("/")[2], response_json_dict.get("assembly", ""), response_json_dict.get("file_type", ""), response_json_dict.get("output_type", ""),response_json_dict2.get("description","")]
